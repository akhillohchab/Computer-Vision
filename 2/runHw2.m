...
signAcademicHonestyPolicy('Akhil Lohchab', 'al3372');
...
for i = 1:length(img_list)
    ...
    rho_num_bins = 2*ceil(sqrt(size(img,1)^2 + size(img,2)^2))
...
img_list = {'hough_1', 'hough_2', 'hough_3'};
hough_threshold = [107 86 101];
% hough_threshold = [0.6 0.6 0.4];

for i = 1:length(img_list)
    orig_img = imread([img_list{i} '.png']);
    hough_img = imread(['accumulator_' img_list{i} '.png']);
%     hough_threshold(i) = hough_threshold(i)*(max(max(hough_img)));
